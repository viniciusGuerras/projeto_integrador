...
exports.getClassroomReservationByUser = async (req, res) => {
    const registration = req.params.matricula;
    repository.findAllClassroomReservationsFromUser(registration)
        .then((fetchedClassroomReservation) => {
            if (!fetchedClassroomReservation) {
                res.status(404).json({ error: "Sala não encontrada" });
            }
            else {
                res.status(200).json({ message: "Sala recuperada com sucesso", classreservations: fetchedClassroomReservation });
            }
        })
        .catch((err) => {
            console.error("Erro ao buscar sala pelo número:", err);
